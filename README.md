...
2. 支持自定义场景对象自由摆放(**目前只支持静态摆放，不支持动态创建，后续可以支持导出清理摆放对象后导出数据支持动态创建场景对象**)。
3. 支持自定义**埋点数据**和**自由摆放埋点数据**进行数据埋点编辑。
4. 支持自定义调整场景大小以及场景地形指定和大小自动适配。
5. 场景数据和埋点数据支持导出自定义数据格式(**目前只支持Json，可自行可做不同数据导出格式**)。
...
10. **场景对象**相关数据通过挂在**MapObjectDataMono**脚本存储相关对象数据。
11. **埋点数据**通过导出时**导出自定义配置数据**实现自定义数据导出。
12. 导出前通过**Map.cs**存储的数据构建自定义数据(**MapExport.cs**)实现自定义数据导出
13. 大地图暂定通过分块拼凑的方式组装按需加载。
...
2. **目前只支持静态摆放，不支持动态创建，后续可以支持导出清理摆放对象后导出数据支持动态创建场景对象**
...
1. **因为静态地图对象是跟随预制件一起加载的，所以导出的地图对象数据只包含埋点数据相关的**
2. **扩展自定义配置数据需要自行扩展或继承MapObjectData或MapData和MapEditor.cs面板显示**
3. **扩展自定义导出埋点数据需要自行扩展或继承BaseMapDataExportt定义**
...
埋点数据根据摄像机区域动态创建显示和销毁:

![DynamicCreateMapData1](img/Unity/MapEditor/DynamicCreateMapData1.PNG)

![DynamicCreateMapData2](img/Unity/MapEditor/DynamicCreateMapData2.PNG)
